...
void ReadSensor() {
  ...
  {
    while (isnan(htu.readTemperature())) { }  // Make sure there are no reading errors
    Temperature = htu.readTemperature();      // Read the current temperature
    if (Temperature >= 50 || Temperature < -30) Temperature = LastTemperature; // Check and correct any errorneous readings
    LastTemperature = Temperature;
    while (isnan(htu.readHumidity())) { }     // Make sure there are no reading errors
...
void StartSensor() {
  ...
  if (!simulating) {                               // If not sensor simulating, then start the real one
    bool status = htu.begin();                     // Start sensor
    if (!status) {
