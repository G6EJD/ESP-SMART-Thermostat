...
#define RelayPIN        D1             // Define the Relay Control pin
...
void setup() {
  ...
  StartLittleFS();                        // Start LittleFS filing system, update to SPIFFS
...
void AssignSensorReadingsToArray() {
  ...
  AddReadingToSensorData(1, Temperature, Humidity); // Only sensor-1 is implemented here, ccould  be more thhough
}
//#########################################################################################
...
void SetupSystem() {
  ...
  delay(200);
  Serial.println(__FILE__);                          // Display file version
...
boolean SetupTime() {
  ...
  tzset();
  int timeToStart = millis() + 5000;
  while (millis() < timeToStart) {                   // Wait for time to start, can take many seconds on the ESP8266, maybe 15-secs
    UpdateLocalTime();
    delay(200);
  }
}
...
void UpdateLocalTime() {
  ...
  Time_str = time_output;             // returns 12:31
  strftime(time_output, 30, "%w", localtime(&now));
  DoW_str  = time_output;             // returns 0 - 6
  UnixTime = now;
}
//#########################################################################################
String ConvertUnixTime(int unix_time) {
  ...
  time_t now;
  now = unix_time;
  strftime(time_output, sizeof(time_output), "%H:%M", localtime(&now));
  return time_output;
}
//#########################################################################################
void StartLittleFS() {
  Serial.println("Starting LittleFS");
  boolean LittleFS_Status;
  LittleFS_Status = LittleFS.begin();
  if (LittleFS_Status == false) {
    Serial.println("Formatting LittleFS ...");
    File datafile = LittleFS.open("/" + DataFile, "r");
    if (!datafile || !datafile.isDirectory()) {
      Serial.println("LitleFS failed to start..."); // Nothing more can be done, so delete and then create another file
      LittleFS.remove("/" + DataFile); // The file is corrupted!!
      datafile.close();
    }
  }
  else Serial.println("LittleFS Started successfully...");
}
//#########################################################################################
...
//#########################################################################################
void StartSensor() {
  Wire.begin(D4, D3);                              // Default SDA and SCL pins
